...
if ($action === 'add' && $_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // หากว่าง ให้ใช้ NULL แทน 
    $max_days = empty($_POST['max_days_per_year']) ? 'NULL' : (int)$_POST['max_days_per_year']; 

    $conn->begin_transaction();
    try {
        // ใช้ Prepared Statement เพื่อความปลอดภัย
        $sql = "INSERT INTO LEAVE_TYPE (leave_type_name, max_days_per_year) VALUES (?, $max_days)";
...
// ------------------ READ (Single Item for Edit/Delete) ------------------
$type_data = null;
if ($type_id > 0) {
    $sql_read = "SELECT * FROM LEAVE_TYPE WHERE leave_type_id = ?";
    if ($stmt_read = $conn->prepare($sql_read)) {
        $stmt_read->bind_param("i", $type_id);
        $stmt_read->execute();
...
if ($action === 'edit' && $_SERVER["REQUEST_METHOD"] === "POST" && $type_data) {
    ...
    $conn->begin_transaction();
    try {
        $sql = "UPDATE LEAVE_TYPE SET leave_type_name=?, max_days_per_year=$max_days WHERE leave_type_id=?";
        if ($stmt = $conn->prepare($sql)) {
...
if ($action === 'delete' && $type_data) {
    $sql_delete = "DELETE FROM LEAVE_TYPE WHERE leave_type_id = ?";
    
    $conn->begin_transaction();
    try {
        if ($stmt_del = $conn->prepare($sql_delete)) {
            $stmt_del->bind_param("i", $type_id);
            $stmt_del->execute();
            $stmt_del->close();
        } else {
             throw new Exception("Error preparing statement: " . $conn->error);
    ...
    <div class="card">
          ...
          <table class="data-table">
            ...
            <?php if($types_result && $types_result->num_rows>0): ?>
              ...
              <tr>
                ...
                <td><?= $row['max_days_per_year'] ? $row['max_days_per_year'] : 'ไม่จำกัด/ไม่ได้กำหนด' ?></td>
                <td>
        ...
        <?php else: ?>
          <form method="post">
            ...
            <div class="form-group">
              <label>วันลาสูงสุดต่อปี (เว้นว่างหากไม่จำกัด)</label>
              <input type="number" name="max_days_per_year" value="<?= htmlspecialchars($type_data['max_days_per_year'] ?? '') ?>">
              </div>
